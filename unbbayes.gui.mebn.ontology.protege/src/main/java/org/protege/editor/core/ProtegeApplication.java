 ...
 * 
 * @version UnBBayes-PR-OWL2 150626 : adapted the application to fit better on non-OSGi application.
...
//                logger.warn("Uncaught Exception in thread " + t.getName(), e);
    ...
    private void loadPlugins() {
        ...
        PluginManager.getInstance().setAutoUpdateEnabled(false);
    }
...
//        }
    ...
    /**
     * Instantiate welcome frame (for backward compatibility), 
     * but don't show it yet, 
     */
    private void showWelcomeFrame(){
    	if (welcomeFrame == null) {
    		welcomeFrame = new JFrame("Dummy frame");
    		welcomeFrame.setVisible(false);
    	}
//        if (welcomeFrame == null){
...
//        welcomeFrame.setVisible(true);
    ...
    public void editURI(URI uri) throws Exception {
		...
		WorkspaceManager workspaceManager = editorKitManager .getWorkspaceManager();
        for (EditorKitFactoryPlugin plugin : pm.getEditorKitFactoryPlugins()) {
            if (plugin.newInstance().canLoad(uri)) {
                ...
                // force the last loaded protege frame to become invisible
                if (editorKitManager.getEditorKitCount() > 0) {
                	workspaceManager.getFrame(editorKitManager.getEditorKits().get(editorKitManager.getEditorKitCount()-1).getWorkspace()).setVisible(false);
                }
                break;
            }
        }
        // force all protege frames to become invisible
        welcomeFrame.setVisible(false);
    }
