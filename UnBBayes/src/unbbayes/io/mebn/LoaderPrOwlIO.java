...
public class LoaderPrOwlIO {
		...
		for (Object owlIndividual : instances){
			...
			try{
			   Type.addType(individualOne.getBrowserText()); 
				...
				//OK... lembre-se que os tipos basicos j?申 existem... 
			}
	...
	private void loadArgRelationship() throws IOMebnException{
		...
		for (Iterator it = instances.iterator(); it.hasNext(); ){	
			...
			if(individualTwo != null){
				//TODO apenas por enquanto, pois n?申o podera ser igual a null no futuro!!!
	...
	private NodeFormulaTree buildFormulaTree(ContextNode contextNode){
		...
		/* 
		 * a raiz sera setada como o builtIn do qual o contextnode ?申 instancia. 
		 * */
		...
		if((obj instanceof BuiltInRV)){
			 ...
			 * acarreta uma busca dos argumentos internos a este at?申 se chegar ao final. 
			 */
