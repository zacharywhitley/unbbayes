        ...
        if (st.sval.equals("net")) {
            ...
            if (st.sval.equals("{")) {
                ...
                while (!st.sval.equals("}")) {
                    ...
                    if (st.sval.equals("node_size")) {
                        proximo(st);
                        proximo(st);
                        net.setRadius(Double.parseDouble(st.sval) / 2);
                    }
                    if (st.sval.equals("HR_Color_Utility")) {
                        proximo(st);
                        UtilityNode.setColor(Integer.parseInt(st.sval));
                    }
                    if (st.sval.equals("HR_Color_Decision")) {
                        proximo(st);
            ...
            if (st.sval.equals("node") || st.sval.equals("decision") || st.sval.equals("utility")) {
                ...
                while (!st.sval.endsWith("}")) {
                    if (st.sval.equals("data")) {
                        ...
                        proximo(st);
                        while (!st.sval.equals("}")) {
                            auxTabPot.addValueAt(nDim++, Double.parseDouble(st.sval));
                            proximo(st);
                        }
                    }
                    else {
        ...
        try {
            ...
            for (int c1 = 0; c1 < sizeNos; c1++) {
                ...
                if (! (auxNo1 instanceof UtilityNode)) {
                    ...
                    arq.println("     states = (" + auxString.toString() + ");");
                }
            ...
            for (int c1 = 0; c1 < net.noVariaveis(); c1++) {
                ...
                arq.println("{");
                if (auxNo1 instanceof ITabledVariable) {
                    PotentialTable auxTabPot = ((ITabledVariable)auxNo1).getPotentialTable();
                    auxListVa.clear();
                    ...
                    */
                    ...
                    for (int c2 = 0; c2 < sizeDados; c2++) {
                        ...
                        if ((c2 % auxNo1.getStatesSize()) == auxNo1.getStatesSize() - 1) {
                            arq.print(" ");
                        }
