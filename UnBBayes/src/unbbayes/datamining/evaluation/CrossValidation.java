...
public class CrossValidation implements ITrainingMode {
	...
	public int getFolds() {
	  ...
	  {   	  
		  data.randomize(data.getRandomNumberGenerator((new Date()).getTime()));
	   ...
	   */
	  protected InstanceSet trainCV(InstanceSet instances, int numFolds, int numFold)
	  {	int numInstForFold, first, offset;
		...
		instances.copyInstances(first + numInstForFold, train, first, numInstances - first - numInstForFold);
	   ...
	   * or greater than the number of instances.
	   */
	  protected InstanceSet testCV(InstanceSet instances,int numFolds, int numFold)
	  { int numInstForFold, first, offset;
		int numInstances = instances.numInstances();
		InstanceSet test;
