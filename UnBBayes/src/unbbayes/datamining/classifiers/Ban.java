...
/**
 * Class implementing an BAN classifier.
 *
 * @author Gabriel
 * @version $1.0
 */
public class Ban extends DistributionClassifier
{	/** Load resources file from this package */
	...
	*/
	...
	{	
		...
		net = new ProbabilisticNetwork("BAN");
		width = 50;
		attIndex=0;
		float sum;
	...
	//	if (bool == true)
		...
		for (i = 0; i < numInstances; i++)
		{
			...
			//if (instances.getClassIndex()>-1)
			{	
				attIndex=0;
				for (j = 0; j < numAtt; j++)
				{
					if ((attributes[j].getIndex() != classIndex)&&(instances.getClassIndex()>-1))
					{//instances.getAttribute(attributes[j]).
						counts[instances.getClassIndex()][attIndex][(int)instance.getByteValue(attributes[j])] += instance.getWeight();
						attIndex++;
					}
...
public void SetBNNet(NodeList net2){
	BNnet= new NodeList();
	BNnet.ensureCapacity(net2.size());
	this.BNnet=net2;
}

private void pegaestrutura(){
