...
public class AttributesTree extends JTree{
	...
	private ResourceBundle resource = ResourceBundle.getBundle("unbbayes" +
			".datamining.gui.resources.GuiResource");

	public AttributesTree() {
		attributesTree = this;
		setShowsRootHandles(true);
		setSelectionModel(null);			//null = n�s n�o selecionaveis
	...
	public Instance getInstance() {
		...
		for (int i = 0; i < keysSize; i++){
			...
			if (obj instanceof StateObject){
				...
				if (check == CHECK_YES){
					instance.setValue(state.getAttribute().getIndex(),
							state.getAttributeValue());
				}
			} else if (obj instanceof TextInputObject) {
				TextInputObject inputText = (TextInputObject) obj;
				instance.setValue(inputText.getAttribute().getIndex(),
						inputText.getAttributeValue());
			}
		}
	...
	private void stateNodeClicked(MouseEvent evt,
		...
		public Component getTreeCellRendererComponent(JTree tree, Object value,
			...
			if (leaf) {
				...
				if (obj instanceof StateObject) {
					...
					setIcon((check == CHECK_YES) ? yesIcon
							: ((check == CHECK_NO) 
									? noIcon : emptyIcon));
				}
			} else {
	...
	private class MyTreeModelListener implements TreeModelListener {
		public void treeNodesChanged(TreeModelEvent e) {
			...
			if (obj instanceof TextInputObject) {
				...
				inferencePanel.printSelectedAttributes(attributesTree
						.getInstance());
			}
		}
