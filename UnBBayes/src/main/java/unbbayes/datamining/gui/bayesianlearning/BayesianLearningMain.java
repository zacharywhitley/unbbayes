...
public class BayesianLearningMain extends JInternalFrame {
	...
	private void jbInit() throws Exception {
		...
		jMenu1.setMnemonic(((Character) resource.getObject("fileMnemonic"))
				.charValue());
		jMenu1.setText(resource.getString("fileMenu"));
		jMenuItem1.setIcon(abrirIcon);
		jMenuItem1.setMnemonic(((Character) resource.getObject("openMnemonic"))
				.charValue());
		...
		jMenuItem2.setMnemonic(((Character) resource.getObject("exitMnemonic"))
				.charValue());
		...
		jMenu2.setMnemonic(((Character) resource.getObject("helpMnemonic"))
				.charValue());
		jMenu2.setText(resource.getString("help"));
		jMenuItem3.setIcon(helpIcon);
		jMenuItem3.setMnemonic(((Character) resource
		...
		jMenu3.setMnemonic(((Character) resource.getObject("learningMnemonic"))
				.charValue());
		jMenu3.setText(resource.getString("learningMenu"));
		jMenuItem4.setEnabled(false);
		jMenuItem4.setIcon(compilaIcon);
		jMenuItem4.setMnemonic(((Character) resource
				.getObject("learnNaiveBayesMnemonic")).charValue());
		...
		jMenuItem5.setMnemonic(((Character) resource
				.getObject("saveNetworkMnemonic")).charValue());
		...
		jPanel8
				.setToolTipText("When field A is indirect cause of field B, select A from the first "
	...
	void jMenuItem4_actionPerformed(ActionEvent e) {
		...
		try {
			algorithmController = new AlgorithmController(inst, ""
			...
			jPanel1.add(algorithmController.getNetWindow().getContentPane(),
					BorderLayout.CENTER);
			statusBar.setText("Estrutura Aprendida");
			jTabbedPane1.setEnabledAt(1, true);
			jTabbedPane1.setSelectedIndex(1);
			net = (ProbabilisticNetwork) algorithmController.getNetWindow()
					.getSingleEntityNetwork();
			saveButton.setEnabled(true);
		} catch (InvalidParentException e1) {
			JOptionPane.showMessageDialog(null, e1.getMessage(), resource
	...
	void jMenuItem1_actionPerformed(ActionEvent e) {
		...
		String[] s1 = { "ARFF" };
		String[] s2 = { "TXT" };
		fileChooser = new JFileChooser(FileController.getInstance()
				.getCurrentDirectory());
		fileChooser.setMultiSelectionEnabled(false);
		// adicionar FileView no FileChooser para desenhar �cones de arquivos
		fileChooser.setFileView(new FileIcon(this));
		fileChooser.addChoosableFileFilter(new SimpleFileFilter(s2,
				"TxtFiles (*.txt)"));
		fileChooser.addChoosableFileFilter(new SimpleFileFilter(s1,
				"ArffFiles (*.arff)"));
		int returnVal = fileChooser.showOpenDialog(this);
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			File selectedFile = fileChooser.getSelectedFile();
			openFile(selectedFile);
			FileController.getInstance().setCurrentDirectory(
	...
	private void openFile(File selectedFile) {
		try {
			jTabbedPane1.setEnabledAt(0, false);
			inst = FileController.getInstance().getInstanceSet(selectedFile,
					this);
			if (inst != null) {
				boolean bool = inst.checkNumericAttributes();
				if (bool == true) {
					throw new Exception(resource
							.getString("numericAttributesException"));
				}
	...
	void jMenuItem5_actionPerformed(ActionEvent e) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				if (!fileName.regionMatches(true, fileName.length() - 4,
						".net", 0, 4)) {
				...
				statusBar.setText(resource
						.getString("errorWritingFileException")
						+ selectedFile.getName() + " " + ioe.getMessage());
			}
			FileController.getInstance().setCurrentDirectory(
					fileChooser.getCurrentDirectory());
