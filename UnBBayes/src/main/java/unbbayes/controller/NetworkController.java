...
public class NetworkController implements KeyListener, INetworkMediator {
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#setInferenceAlgorithm(unbbayes.util
	...
	private static ResourceBundle resource = unbbayes.util.ResourceController
			.newInstance().getBundle(
					unbbayes.controller.resources.ControllerResources.class
							.getName());

	/**
	 * This is the default constructor, initializing nothing. This is made
	 * protected in order to make it easier to extend.
	 */
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#removeEvidence(unbbayes.prs.Node)
	 */
	public void removeEvidence(Node node) {
		if (senController != null)
			senController.removeEvidence(node);
	}
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#createContinuousDistribution(unbbayes
	 * .prs.hybridbn.ContinuousNode)
	 */
	/**
	 * @deprecated Continuous node is no longer supported in UnBBayes core. It
	 *             has now been replaced by the CPS plugin available at
	 *             http://sourceforge.net/projects/prognos/.
	 */
	public void createContinuousDistribution(ContinuousNode node) {
		if (senController != null)
			senController.createContinuousDistribution(node);
	}
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#createDiscreteTable(unbbayes.prs
	 * .Node)
	 */
	public void createDiscreteTable(Node node) {
		if (senController != null)
			senController.createDiscreteTable(node);
	}
	...
	public JTable makeTable(final Node node) {
		if (senController != null)
			return senController.makeTable(node);
		return null;
	}
	...
	public boolean compileNetwork() {
		if (senController != null)
			return senController.compileNetwork();
		return false;
	}
	...
	public void evaluateNetwork() {
		if (singleEntityNetwork != null
				&& singleEntityNetwork instanceof ProbabilisticNetwork) {
			EvaluationController evaluationController = new EvaluationController(
					(ProbabilisticNetwork) singleEntityNetwork);
			screen.changeToPNEvaluationPane(evaluationController.getView());
		} else {
			JOptionPane.showMessageDialog(screen,
					"Evaluation can only be done in probabilistic networks.",
	 ...
	 * (non-Javadoc)
	 * 
	 * @see unbbayes.controller.INetworkMediator#insertContinuousNode(double,
	 * double)
	 */
	public Node insertContinuousNode(double x, double y) {
		if (senController != null)
			return senController.insertContinuousNode(x, y);
		return null;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see unbbayes.controller.INetworkMediator#insertProbabilisticNode(double,
	 * double)
	 */
	public Node insertProbabilisticNode(double x, double y) {
		if (senController != null)
			return senController.insertProbabilisticNode(x, y);
		return null;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see unbbayes.controller.INetworkMediator#insertDecisionNode(double,
	 * double)
	 */
	public Node insertDecisionNode(double x, double y) {
		if (senController != null)
			return senController.insertDecisionNode(x, y);
		return null;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see unbbayes.controller.INetworkMediator#insertUtilityNode(double,
	 * double)
	 */
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#showExplanationProperties(unbbayes
	 * .prs.bn.ProbabilisticNode)
	 */
	public void showExplanationProperties(ProbabilisticNode node) {
		if (senController != null)
			senController.showExplanationProperties(node);
	}
	...
	public boolean insertEdge(Edge edge) throws Exception {
		if (senController != null)
			return senController.insertEdge(edge);
		return false;
	}
	...
	public void insertState(Node node) {
		if (senController != null)
			senController.insertState(node);
	}
	...
	public void removeState(Node node) {
		if (senController != null)
			senController.removeState(node);
	}
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#deleteSelected(java.lang.Object)
	 */
	// by young
	public void deleteSelected(Object selected) {
		if (senController != null)
			senController.deleteSelected(selected);
	}
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#keyTyped(java.awt.event.KeyEvent)
	 */
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#keyPressed(java.awt.event.KeyEvent)
	 */
	public void keyPressed(KeyEvent e) {

		// by young
		/*
		 * if (e.getKeyCode() == KeyEvent.VK_DELETE) { Object selecionado =
		 * screen.getGraphPane().getSelected(); deleteSelected(selecionado);
		 * 
		 * for (int i = 0; i < screen.getGraphPane().getSelectedGroup().size();
		 * i++) { selecionado = screen.getGraphPane().getSelectedGroup().get(i);
		 * deleteSelected(selecionado); } } screen.getGraphPane().update();
		 */
	}
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#keyReleased(java.awt.event.KeyEvent)
	 */
	...
	public void saveNetImage() {
		...
		chooser.addChoosableFileFilter(new SimpleFileFilter(images, resource
				.getString("imageFileFilter")));
...
//		}
	...
	/**
	 * This method is called inside {@link #showLog()} to retrieve the content
	 * of LOG. Extend this method in order to customize the log message (e.g.
	 * customize where the log content is stored, and how to retrieve it)
	 * 
	 * @return a non null string. If {@link #singleEntityNetwork} is null, it
	...
	public JDialog showLog() {
		TextAreaDialog dialog = new TextAreaDialog(this.getScreen()
				.getUnbbayesFrame(), false);
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#previewPrintLog(javax.swing.JTextArea
	 * , javax.swing.JDialog)
	 */
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#previewPrintNet(javax.swing.JComponent
	 * , java.awt.Rectangle)
	 */
	public void previewPrintNet(final JComponent rede, final Rectangle retangulo) {
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				String rotulo = JOptionPane.showInputDialog(screen,
						resource.getString("askTitle"),
						resource.getString("informationText"),
						JOptionPane.INFORMATION_MESSAGE);
				if (rotulo == null) {
					rotulo = "";
				}
				PrintNet it = new PrintNet(rotulo, rede, retangulo,
						new PageFormat());
				PrintPreviewer pp = new PrintPreviewer(it, 0);
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#printNet(javax.swing.JComponent,
	 * java.awt.Rectangle)
	 */
	public void printNet(final JComponent network, final Rectangle rectangle) {
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				PrintNet it = new PrintNet(rotulo, network, rectangle,
						new PageFormat());
	...
	public void printTable() {
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				PrintTable impressora = new PrintTable(tabelas, donos,
						new PageFormat());
	 ...
	 * 
	 * @param textArea
	 *            The text area containing the log.
	 * @deprecated this method was moved to
	 *             {@link unbbayes.gui.util.TextAreaDialog}
	 */
	...
	public Rectangle calculateNetRectangle() {
		...
		// by young
		/*
		 * ArrayList<Node> nos; List vetorAux =
		 * screen.getGraphPane().getSelectedGroup();
		 * 
		 * if (vetorAux.size() == 0) { nos = new ArrayList<Node>(); for (int i =
		 * 0; i < singleEntityNetwork.getNodeCount(); i++) { nos.add(i,
		 * singleEntityNetwork.getNodeAt(i)); } } else { nos = new
		 * ArrayList<Node>(); for (int i = 0; i < vetorAux.size(); i++) { if
		 * (vetorAux.get(i) instanceof Node) { nos.add((Node)vetorAux.get(i)); }
		 * } } int maiorX = 0; int menorX = Integer.MAX_VALUE; int maiorY = 0;
		 * int menorY = Integer.MAX_VALUE; Node noAux; Point2D pontoAux; int
		 * xAux; int yAux; for (int i = 0; i < nos.size(); i++) { noAux =
		 * (Node)nos.get(i); pontoAux = noAux.getPosition(); xAux =
		 * (int)pontoAux.getX(); yAux = (int)pontoAux.getY(); if (xAux > maiorX)
		 * { maiorX = xAux; } if (xAux < menorX) { menorX = xAux; } if (yAux >
		 * maiorY) { maiorY = yAux; } if (yAux < menorY) { menorY = yAux; } }
		 * //by young
		 * 
		 * double nodeWidth = noAux.getWidth(); maiorX += nodeWidth; maiorY +=
		 * nodeWidth; menorX -= nodeWidth; menorY -= nodeWidth;
		 * 
		 * return new Rectangle(menorX, menorY, maiorX - menorX, maiorY -
		 * menorY);
		 */

		return new Rectangle(0, 0, (int) screen.getGraphPane()
				.getBiggestPoint().x, (int) screen.getGraphPane()
				.getBiggestPoint().y);
	 ...
	 * 
	 * @see
	 * unbbayes.controller.INetworkMediator#setScreen(unbbayes.gui.NetworkWindow
	 * )
	 */
