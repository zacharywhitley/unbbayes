...
public class JavaHelperController {
	...
	{	 component.setCursor(new Cursor(Cursor.WAIT_CURSOR));
		...
		if (className.equals("unbbayes.datamining.gui.id3.DecisionTreeMain")){	 
			set = new HelpSet(null, getClass().getResource("/help/DataMiningHelp/Decision_Tree.hs"));
			find = true; 
		}
		
		if (className.equals("unbbayes.datamining.gui.c45.DecisionTreeMain")){	 
			set = new HelpSet(null, getClass().getResource("/help/C45Help/C45.hs"));
			find = true; 
		}
		
		if (className.equals("unbbayes.datamining.gui.evaluation.EvaluationMain")){	 
			set = new HelpSet(null, getClass().getResource("/help/DataMiningHelp/Evaluation.hs"));
			find = true; 
		}
		
		if (className.equals("unbbayes.datamining.gui.naivebayes.NaiveBayesMain")){	 
			set = new HelpSet(null, getClass().getResource("/help/DataMiningHelp/Naive_Bayes.hs"));
			find = true; 
		}
		
		if (className.equals("unbbayes.datamining.gui.preprocessor.PreprocessorMain")){	 
			set = new HelpSet(null, getClass().getResource("/help/DataMiningHelp/Preprocessor.hs"));
			find = true; 
		}
		
		if (className.equals("unbbayes.datamining.gui.neuralmodel.NeuralModelMain")){	 
			set = new HelpSet(null, getClass().getResource("/help/CNMHelp/cnm.hs"));
			find = true; 
		}
		
		if (className.equals("unbbayes.datamining.gui.neuralnetwork.NeuralNetworkMain")){	 
			set = new HelpSet(null, getClass().getResource("/help/BpnHelp/BpnHelp.hs"));
			find = true; 
		}
		
		if (className.equals("unbbayes.gui.UnBBayesFrame")){	 
