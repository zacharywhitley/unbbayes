...
public class MetaphorResult extends JPanel {
	...
	private void jbInit() throws Exception {
		...
		jLabel1.setText("Prov�veis Diagn�sticos:");
		jLabel2.setText("Diagn�sticos N�o Prov�veis:");
		...
		jTabbedPane1.add(jScrollPane3, "Descri��o do Diagn�stico:");
	...
	public void updateResults() {
		...
		if (explanationNodes != null) {
			...
			for (int i = 0; i < size; i++) {
				ProbabilisticNode node = (ProbabilisticNode) explanationNodes
						.get(i);
				int statesSize = node.getStatesSize();
				if (statesSize > 1) {
					listModel1.addElement(new ListObject(node.getDescription(),
							node.getExplanationDescription()));
					listModel2.addElement(new ListObject(node.getDescription(),
							node.getExplanationDescription()));
					for (int j = 0; j < statesSize; j++) {
						double marginal = node.getMarginalAt(j);
						String tmp = String.format("        %s        %.2f%%", node.getStateAt(j), marginal * 100);
						if (marginal >= 0.5) {
							listModel1.addElement(new ListObject(tmp, node, ListObject.FINDINGS));
						} else {
							listModel2.addElement(new ListObject(tmp, node, ListObject.WHY_NOT));
						}
					}
				}
			}
	...
	private class ListObject {
		...
		public ListObject(String name, ProbabilisticNode node, boolean flag) {
			...
			for (int i = 0; i < size; i++) {
				phrases[i] = (ExplanationPhrase) node.getPhrasesMap().get(
						keys.get(i));
			}
			if (flag == FINDINGS) {
	...
	void jList_valueChanged(ListSelectionEvent e) {
		if (flag) {
			...
			if (source.equals(jList1)) {
				...
				jTabbedPane1.setTitleAt(1, "Por que n�o?");
			}
