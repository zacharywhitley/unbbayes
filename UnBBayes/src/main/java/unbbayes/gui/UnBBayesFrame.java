...
/**
 * This class extends <code>JFrame</code> and it is responsible for the UnBBayes
 * main panel.
 * 
...
public class UnBBayesFrame extends JFrame {
	...
	private UnBBayesPluginContextHolder unbbayesPluginContextHolder = UnBBayesPluginContextHolder
			.newInstance();
	...
	public UnBBayesFrame(MainController _controller) {
		...
		this.setModuleCategoryToComponentsMap(this
	...
	protected void updatePluginMenuVisibility() {
		...
		if ((this.getPluginMenu().getPopupMenu().getComponents() == null)
				|| (this.getPluginMenu().getPopupMenu().getComponents().length <= 2)) { // at
																						// least
																						// "reload and split"
	...
	private void setLnF(String lnfName) {
		try {
			...
			System.err.println(resource
					.getString("LookAndFeelUnsupportedException") + lnfName);
		} catch (ClassNotFoundException ex2) {
			System.err.println(resource
					.getString("LookAndFeelClassNotFoundException") + lnfName);
		} catch (InstantiationException ex3) {
			System.err.println(resource
					.getString("LookAndFeelInstantiationException") + lnfName);
		} catch (IllegalAccessException ex4) {
			System.err.println(resource
					.getString("LookAndFeelIllegalAccessException") + lnfName);
		}
	}
	...
	public void createActionListeners() {
		...
		// int classe = 0;
		// chooser = new
		// JFileChooser(fileHistoryController.getCurrentDirectory());
		...
		alSave = new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				String filterMessage = null; // message to be added to file
												// filter
				String dialogueTitle = null; // title of the file chooser
				JInternalFrame focusedInnerFrame = desktop.getSelectedFrame(); // currently
																				// focused
																				// window
				...
				if (focusedInnerFrame != null) {
					if (focusedInnerFrame instanceof UnBBayesModule) {
						...
						filterMessage = currentWindow
								.getSupportedFilesDescription(false);

						file = currentWindow.getFile();
						if (file == null) {
							file = chooseFile(filterMessage, nets);
		...
		// Create an action listener for saving as.
		alSaveAs = new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				setCursor(new Cursor(Cursor.WAIT_CURSOR));
	 ...
	 * 
	 * @param filterMessage
	 *            message for the filter in the file chooser.
	 * @param nets
	 *            filter supported for file types.
	 * @return
	 */
	...
	/*
	 * Saves a UnBBayes module in a File.
	 * 
	 * @param currentWindow module for saving
	 * 
	 * @param file file to save the module.
	 */
	protected void saveNetwork(UnBBayesModule currentWindow, File file) {
		try {
			...
			// OBS.
			// FileExtensionIODelegator.MoreThanOneCompatibleIOException is
			// managed
			// differently by load and save. This is because load is done by
			// the module and the
			// save is done by IO (on load, we are not sure what module to
			// use, but on save we
			// are sure that we should use the currently active
			// module/window).
			// Thats why MoreThanOneCompatibleIOException must be treated by
			// modules on load,
			// but treated by UnBBayesFrame on save.

			// More than one I/O was found to be compatible. Ask user to
			// select one.
			String[] possibleValues = FileExtensionIODelegator
					.getNamesFromIOs(more.getIOs());
			...
			if (selectedValue != null) {
				BaseIO io = FileExtensionIODelegator.findIOByName(
						more.getIOs(), selectedValue);
				try {
					if (controller.saveNet(file, io,
							currentWindow.getPersistingGraph())) {
						// save successful
						JOptionPane.showMessageDialog(UnBBayesFrame.this,
								resource.getString("saveSucess"),
								resource.getString("sucess"),
	...
	public void createMenu() {
		...
		recentFilesMenu.setMnemonic(resource.getString("recentFilesMn").charAt(
				0));
		...
		JMenuItem saveAsItem = new JMenuItem(resource.getString("saveAsItem"),
				iconController.getSaveIcon());
		...
		JMenuItem tbFile = new JCheckBoxMenuItem(resource.getString("tbFile"),
				true);
		JMenuItem tbNewProject = new JCheckBoxMenuItem(
				resource.getString("tbNewProject"), true);
		...
		JMenuItem tbWindow = new JCheckBoxMenuItem(
				resource.getString("tbWindow"), true);
		JMenuItem tbHelp = new JCheckBoxMenuItem(resource.getString("tbHelp"),
				true);

		// Check box item for plugin toolbar
		JMenuItem tbPlugin = new JCheckBoxMenuItem(
				resource.getString("tbPlugin"), true);
		...
		JMenuItem homeItem = new JMenuItem(resource.getString("windowsItem"),
				iconController.getHomeIcon());
		JMenuItem cascadeItem = new JMenuItem(
				resource.getString("cascadeItem"),
				iconController.getCascadeIcon());
		...
		newBN.setAccelerator(KeyStroke.getKeyStroke(
				resource.getString("newItemMn").charAt(0), Event.CTRL_MASK,
				false));
		openItem.setAccelerator(KeyStroke.getKeyStroke(
				resource.getString("openItemMn").charAt(0), Event.CTRL_MASK,
				false));
		saveItem.setAccelerator(KeyStroke.getKeyStroke(
				resource.getString("saveItemMn").charAt(0), Event.CTRL_MASK,
				false));
		saveAsItem.setAccelerator(KeyStroke.getKeyStroke(
				resource.getString("saveAsItemMn").charAt(0), Event.CTRL_MASK,
				false));
		helpItem.setAccelerator(KeyStroke.getKeyStroke(
				resource.getString("helpItemMn").charAt(0), Event.CTRL_MASK,
				false));

		// Learning menu itens
		// JMenuItem learningItem = new
		// JMenuItem(resource.getString("learningItem"),
		// iconController.getLearningIcon());
		...
		// JMenuItem lwItem = new
		// JMenuItem(resource.getString("likelihoodWeightingItem"));
		// JMenuItem gibbsItem = new JMenuItem(resource.getString("gibbsItem"));
		// JMenuItem iLearningItem = new
		// JMenuItem(resource.getString("ILearningItem"));
		// JMenuItem metaphorItem = new
		// JMenuItem(resource.getString("MetaphorItem"));
		// JMenuItem medicalMetaphorItem = new
		// JMenuItem(resource.getString("MedicalMetaphorItem"));
		// JMenuItem unbMinerItem = new
		// JMenuItem(resource.getString("UnBMinerItem"));
		...
		saveAsItem.addActionListener(alSaveAs);
		...
		ConfigurationsController configController = ConfigurationsController
				.getInstance();
		for (String nameOfFile : configController.getConfigurations()
				.getPreviewOpenFiles()) {
			File file = new File(nameOfFile);
			JMenuItem fileMenuItem = new JMenuItem(nameOfFile);
			fileMenuItem
					.addActionListener(new OpenFileUsingModulesActionListener(
							file));
	...
	/**
	 * This method is called inside the constructor in order to initialize the
	 * map, which is to be stored at {@link #getModuleCategoryToComponentsMap()}
	 ...
	 * @return the builded map. The caller must use this map to set
	 *         {@link #setModuleCategoryToComponentsMap(Map)}
	 */
	...
	/**
	 * This method is used whithin {@link #loadPlugins()} in order to
	 * instantiate the "Plugin" tool bars. It does not actually fill the tool
	 * bar, since the plugins must be loaded after the creation of the toolbar.
	 * It is only a initializer. The contents are created at
	 * {@link #fillCorePluginMenuAndButtons(PluginManager, ExtensionPoint)}
	 */
	protected void createPluginToolBars() {
		...
		this.getPluginSplitButton().setToolTipText(
				resource.getString("pluginMenu"));

		topPanel.add(pluginToolBar);
	}

	/**
	 * This method is used whithin {@link #loadPlugins()} in order to create the
	 * "Plugin" menu. It does not actually fill the menu with plugins, since the
	 * plugins must be loaded after the creation of the menu. It is only a
	 * initializer. The contents are created at
	 * {@link #fillCorePluginMenuAndButtons(PluginManager, ExtensionPoint)}
	 */
	protected void createPluginMenu() {
		...
		reloadPluginsMenuItem = new JMenuItem(
				this.resource.getString("reloadPlugin"));
		reloadPluginsMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				reloadPlugins();
			}
		});
		reloadPluginsMenuItem.setToolTipText(this.resource
				.getString("reloadPluginToolTip"));
		...
		this.getModuleCategoryToComponentsMap().put(
				PluginCore.PARAMETER_CATEGORY_VALUE_PLUGIN, componentList);
	}

	/**
	 * This method is called when we press the "reload plugin" menu. By
	 * overwriting this method, you can customize the plugin's reload action.
	 * This method basically resets the components obtained from
	 * {@link #getMapOfComponentsToBeRemovedAtPluginReload()}, resets the menu
	 * bar, refills the {@link #getModuleCategoryToComponentsMap()}, reloads
	 * plugins, updates the menu's visibility (if no plugin is present, some
	 * menus and toolbars are hidden), and finally notifies reload action to
	 * {@link UnBBayesPluginContextHolder#notifyReload(Object)} using this
	 * UnBBayesFrame as parameter.
	 */
	protected void reloadPlugins() {
		// reset components
		this.createMenu(); // reset menu
		this.removePluginButtonsFromOtherComponents(this
				.getMapOfComponentsToBeRemovedAtPluginReload()); // remove
																	// buttons
																	// from
																	// other
																	// JTBs
		this.setModuleCategoryToComponentsMap(this
				.buildModuleCategoryToComponentsMap()); // fills up the
														// moduleCategoryToJMenuMap
		this.loadPlugins(); // reload plugins
		this.updatePluginMenuVisibility(); // show menus if it is not empty
											// again

		// reloads every plugin managed by UnBBayesPluginContextHolder by
		// notifying it
	...
	/**
	 * Obtains the classes of modules' builders (UnBBayesModuleBuilder) which
	 * are part of core implementation of unbbayes. Core implementation modules
	 * are instances of UnBBayesModule that was not loaded by plugin mechanism.
	 * 
	...
	public Map<String, Class> getCoreUnBBayesModulesClasses() {
		...
		// adding PN module
		try {
			ret.put(this.resource.getString("PNModuleName"),
					NetworkWindowBuilder.class);
		} catch (Exception e) {
			throw new RuntimeException(
					this.resource.getString("moduleLoadingError")
	...
	/**
	 * This method displays a dialog if a plugin with dependency error is
	 * detected.
	 */
	public void showPluginDependencyError() {
		try {
			...
			Map<String, Set<String>> errorMap = this
					.getUnbbayesPluginContextHolder()
					.getErroneousPluginIDDependencyMap();
			if (!errorMap.isEmpty()) {
				// if there are errors, report using text logs
				TextAreaDialog dialog = new TextAreaDialog(this,
						this.resource.getString("pluginDependencyLogTitle"),
						false);

				// prepare dialog content's header
				String dialogContent = "=============================================================\n";
				dialogContent += this.resource
						.getString("pluginDependencyLogKeyMessage");
				dialogContent += "\n=============================================================\n";
				...
				for (String pluginID : errorMap.keySet()) {
					dialogContent += "\n\n";
					dialogContent += "-> "
							+ this.resource.getString("pluginDependencyLogID");
					dialogContent += " \"" + pluginID + "\"";
					dialogContent += "\n     "
							+ this.resource
									.getString("pluginDependencyLogDependencies");
				...
				dialogContent += "\n=============================================================\n";
				dialogContent += this.resource
						.getString("pluginDependencyLogPleaseCheck");
				dialogContent += "\n=============================================================\n";
	...
	/**
	 * Creates the plugin manager, loads UnBBayes' core plugins using JPF, fills
	 * the menu items and tool bar's buttons and their listeners.
	 */
	public void loadPlugins() {
		...
		// loads the "core" plugin, which is a stub that we use to declare
		// extension points for core
		PluginDescriptor core = this
				.getUnbbayesPluginContextHolder()
				.getPluginManager()
				.getRegistry()
				.getPluginDescriptor(
		...
		ExtensionPoint point = this
				.getUnbbayesPluginContextHolder()
				.getPluginManager()
				.getRegistry()
				.getExtensionPoint(core.getId(),
						this.getPluginModuleExtensionPoint());

		// create menu/buttons that activates a plugin and stores plugin classes
		// to a list
		this.getPluginMap().putAll(
				this.fillCorePluginMenuAndButtons(this
						.getUnbbayesPluginContextHolder().getPluginManager(),
						point));
	 ...
	 *            : manager to use
	 * @param point
	 *            : extension point to be read in order to fill menu and
	 *            buttons.
		...
		for (Extension ext : point.getConnectedExtensions()) {
			try {
				...
				Parameter nameParam = ext
						.getParameter(PluginCore.PARAMETER_NAME);
				Parameter classParam = ext
						.getParameter(PluginCore.PARAMETER_CLASS);
				Parameter descriptionParam = ext
						.getParameter(PluginCore.PARAMETER_DESCRIPTION);
				Parameter iconParam = ext
						.getParameter(PluginCore.PARAMETER_ICON);
				Parameter builderParam = ext
						.getParameter(PluginCore.PARAMETER_BUILDER);
				Parameter categoryParam = ext
						.getParameter(PluginCore.PARAMETER_CATEGORY);
				...
				if (builderParam != null) {
					pluginOrBuilderCls = classLoader.loadClass(builderParam
							.valueAsString());
				} else {
					pluginOrBuilderCls = classLoader.loadClass(classParam
							.valueAsString());
				}
				...
				ImageIcon icon = null;
				if (iconParam != null) {
					URL iconUrl = pluginManager.getPluginClassLoader(
							ext.getDeclaringPluginDescriptor()).getResource(
							iconParam.valueAsString());
				...
				ActionListener listener = new ActionListenerSupportingConstructorParam(
						pluginOrBuilderCls) {
					public void actionPerformed(ActionEvent e) {
						setCursor(new Cursor(Cursor.WAIT_CURSOR));
				...
				// adding menu item into menu, depending on the category of the
				// module
				List<JComponent> mappedElements = this
						.getModuleCategoryToComponentsMap().get(
								categoryParam.valueAsString());
				if (mappedElements == null || mappedElements.isEmpty()) {
					// if the mapping was empty because
					// categoryParam.valueAsString() was not found, then use
					// default
					if (categoryParam.valueAsString() == null
							|| (categoryParam.valueAsString().length() == 0)) {
						mappedElements = new ArrayList<JComponent>();
						mappedElements.add(this.getPluginMenu()); // use default
																	// plugin
																	// menu
					} else {
						// the module category was not mapped before. Create and
						// Map it.
						mappedElements = new ArrayList<JComponent>();
						JMenu newMenu = new JMenu(categoryParam.valueAsString()); // create
																					// new
																					// menu
																					// using
																					// name
				...
				// flag that tells us if we should add a menu item into the
				// default plugin menu.
				boolean addToPluginMenuFlag = true;

				// flag that tell us if we should add a button into default
				// plugin split button
				...
				for (JComponent comp : mappedElements) {
					if (comp instanceof JMenu) {
						...
						JMenuItem splitButtonMenuItem = new JMenuItem(
								moduleName, icon);
						splitButtonMenuItem.setToolTipText(descriptionParam
								.valueAsString());
						splitButtonMenuItem
								.addActionListener(new SplitButtonMenuActionListener(
										spButton, button));

						// adding the plugin button into split button instead of
						// the tool bar itself
						spButton.add(button, splitButtonMenuItem);// the last
																	// loaded
																	// plugin
																	// will be
																	// at top
						...
						// by storing the component and the container, we can
						// remove them at "reload" operation
						this.getMapOfComponentsToBeRemovedAtPluginReload().put(
								splitButtonMenuItem, spButton);
					} else {
						// if we are not adding to a JMenu, add a button instead
						// of menu item
						comp.add(button);
						addToSplitButton = false;

						// by storing the components and its container, we can
						// remove them at "reload" operation
						this.getMapOfComponentsToBeRemovedAtPluginReload().put(
								button, comp);
					}
				...
				if (addToSplitButton) {
					// creating a menu item for split button, only if the flag
					// is on
					JMenuItem splitButtonMenuItem = new JMenuItem(moduleName,
							icon);
					splitButtonMenuItem.setToolTipText(descriptionParam
							.valueAsString());
					splitButtonMenuItem
							.addActionListener(new SplitButtonMenuActionListener(
									this.getPluginSplitButton(), button));

					// adding the plugin button into split button instead of the
					// tool bar itself
					this.getPluginSplitButton()
							.add(button, splitButtonMenuItem); // the last
																// loaded plugin
																// will be at
																// top
	...
	/**
	 * Extracts the module's name from its class (the class can be a builder or
	 * the module itself). Usually, we use
	 * {@link UnBBayesModule#getModuleName()} or
	 * {@link UnBBayesModuleBuilder#getName()} to do so.
	 * 
	 * @param pluginOrBuilderCls
	 *            a class of {@link UnBBayesModule} or
	...
	protected String getUnBBayesModuleNameByPluginClass(Class pluginOrBuilderCls) {
		try {
			if (UnBBayesModuleBuilder.class
				...
				UnBBayesModuleBuilder builder = (UnBBayesModuleBuilder) pluginOrBuilderCls
						.newInstance();
				return builder.getName();
			} else {
				return ((UnBBayesModule) pluginOrBuilderCls.newInstance())
						.getName();
			}
		} catch (Throwable e) {
			e.printStackTrace();
	...
	/**
	 * Use the parameter "clazz" in order to instantiate a new UnBBayesModule
	 * (in this case, "clazz" must provide a default constructor) or as a
	 * UnBBayesModuleBuilder (which by
	 * {@link UnBBayesModuleBuilder#buildUnBBayesModule()} we get the proper
	 * UnBBayesModule).
	 * 
	 * @param clazz
	 *            : class extending {@link UnBBayesModule} or implementing
	 *            {@link UnBBayesModuleBuilder}
	...
	public UnBBayesModule getUnBBayesModuleByPluginClass(Class clazz)
		...
		if (UnBBayesModuleBuilder.class.isAssignableFrom(clazz)) {
			UnBBayesModuleBuilder builder = (UnBBayesModuleBuilder) clazz
					.newInstance();
	 ...
	 * bars and add the buttons to them and finally to the topPanel. TODO fix
	 * the help toolbar
	 */
	public void createToolBars() {
		...
		this.getToolsSplitButton().setToolTipText(
				resource.getString("toolsMenu"));
	...
	/**
	 * This method will use map in order to remove all keys from its mapped
	 * object. In other words, if a button was once added into a jcomponent
	 * within map, the button will be removed from the component and the map.
	 * 
	 * @param map
	 */
	protected void removePluginButtonsFromOtherComponents(
			Map<JComponent, JComponent> map) {
	 ...
	 * The main extension point of the core plugin (Module).
	 * 
	 * @param pluginModuleExtensionPoint
	 *            the module's extension ID to set
	 */
	 ...
	 * 
	 */
	protected abstract class ActionListenerSupportingConstructorParam implements
			ActionListener {
	...
	/**
	 * Connected, loaded and activated extensions. This map associates an ID to
	 * a {@link UnBBayesModule} or {@link UnBBayesModuleBuilder}
	 * 
	...
	/**
	 * @param reloadPluginsMenuItem
	 *            the reloadPluginsMenuItem to set
	 */
	...
	/**
	 * Obtains all modules that can treat a specific file (it looks mainly at
	 * file extension) at loading time. We do not need this method at saving
	 * time since {@link IPersistenceAwareWindow#getIO()} can do the job by
	 * using {@link BaseIO#save(File, unbbayes.prs.Graph)}.
	 * 
	 * @param file
	 *            : file to be loaded.
	 * @param plugins
	 *            : a map of classes extending UnBBayesModule. The keys are the
	 *            plugin/module IDs.
	 * @return a map of instantiated UnBBayesModule, with setVisible(false). The
			...
			for (String id : plugins.keySet()) {
				try {
					UnBBayesModule module = this
							.getUnBBayesModuleByPluginClass(plugins.get(id));
					module.setVisible(false);
					if (module.getIO() != null
							&& module.getIO().supports(file, true)) {
						ret.put(id, module);
					}
				} catch (Exception e) {
					e.printStackTrace();
	...
	/**
	 * Obtains all file extensions (with no dots) that a list of plugins can
	 * handle.
	 * 
	 * @param plugins
	 *            : a list of classes extending UnBBayesModule or its builder
	 *            (UnBBayesModuleBuilder)
	 * @return array of string containing all file extensions supported
	...
	/**
	 * A listener for opening an existing file using plugins/modules. Can be
	 * used in 2 ways: 1 - a file is not specified (in this case a file chooser
	 * will be shown to user); 2 - a file is specified (in this case the
	 * listener will just open the file)
	 * 
	 * @author Shou Matsumoto
	 * 
	 */
	public class OpenFileUsingModulesActionListener implements ActionListener {
		...
		/**
		 * Create a listener in order to open a file treating modules'
		 * conflicts.
		 * 
		 * @param file
		...
		/**
		 * Shows a file chooser to the user, using filters provided by
		 * plugins/modules.
		 * 
		...
		public File showFileChooser() {
			chooser = new JFileChooser(
					fileHistoryController.getCurrentDirectory());
			...
			for (Class clazz : getPluginMap().values()) {
				...
				chooser.addChoosableFileFilter(new SimpleFileFilter(mod
						.getSupportedFileExtensions(true), mod
						.getSupportedFilesDescription(true)));
		 ...
		 * I not, a file chooser will be displayed to user and that file will be
		 * opened using an appropriate UnBBayesModule.
		 */
		public void actionPerformed(ActionEvent ae) {
			setCursor(new Cursor(Cursor.WAIT_CURSOR));
			...
			if (currentFile != null) {
				...
				Map<String, UnBBayesModule> moduleMap = getUnBBayesModulesByFile(
						currentFile, getPluginMap());
				...
				if (moduleMap.size() == 1) {
					...
					// if there are more than one module to handle the file, the
					// user shall choose one
					String[] possibleValues = moduleMap.keySet().toArray(
							new String[moduleMap.keySet().size()]);
				...
				// if we reached this code and no module was selected, report
				// error and stop
	...
	/**
	 * @param unbbayesPluginContextHolder
	...
	/**
	 * This is the split button containing buttons to start plugin-based
	 * modules. This is usually inserted inside {@link #getPluginToolBar()}
	 * 
	 ...
	 * modules. This is usually inserted inside {@link #getPluginToolBar()}
	 * 
	 * @param pluginSplitButton
	 *            the pluginSplitButton to set
	 */
	...
	/**
	 * This is just an action listener for menu items inserted within the
	 * {@link UnBBayesFrame#getPluginSplitButton()} or
	 * {@link UnBBayesFrame#getToolsSplitButton()}.
	 * 
	 * @author Shou Matsumoto
	 * 
	 */
	protected class SplitButtonMenuActionListener implements ActionListener {
		...
		// the button to be set as the split button's main button, after this
		 ...
		 */
		public SplitButtonMenuActionListener(SplitToggleButton splitButton,
				JButton mainButton) {
	...
	/**
	 * This is a sub menu where functionalities like "new BN" or "new MSBN"
	 * resides.
	 * 
	 * @return newMenu
	 */
	 ...
	 * It maps a module category into a menu
	 * 
	 * @param moduleCategoryToComponentsMap
	 *            the moduleCategoryToJMenuMap to set
	 */
	 ...
	 * This separator separates the "reload plugin" option from other plugins
	 * 
	 * @param pluginMenuSeparator
	 *            the pluginMenuSeparator to set
	 */
	...
	/**
	 * It stores all modules' components which was not added into the default
	 * plugin split button. They need special attention, since they are usually
	 * situated at other JTB, and must be removed at plugin reload. The buttons
	 * are mapped to the component where they were added.
	 * 
	 ...
	 * are mapped to the component where they were added.
	 * 
	 * @param mapOfComponentsToBeRemovedAtPluginReload
	 *            the mapOfComponentsToBeRemovedAtPluginReload to set
	 * @see #getJtbFile()
	 */
