...
public class FullSSBNGenerator extends LaskeySSBNGenerator {
	...
	protected void compileAndInitializeSSBN(SSBN ssbn) throws Exception {
		...
		// At least rename nodes, so that invalid characters are not present
		this.renameInvalidNodeNames(ssbn.getNetwork());
	}
	
	/**
	 * This method is called by {@link #compileAndInitializeSSBN(SSBN)} in order
	 * to detect invalid node names (due to presence of potentially invalid letters/characters) and rename them.
	 * A character that is know to be invalid, but frequently present, is the colon (:).
	 * @param network
	 */
	protected void renameInvalidNodeNames(Network network) {
		for (Node node : network.getNodes()) {
			String name = node.getName();
			if (name.contains(":")) {
				Debug.println(getClass(), "Renaming node " + name);
