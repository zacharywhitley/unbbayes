...
public class MEBNPluginNodeManager extends CorePluginNodeManager {
	...
	// ID of UnBBayes-MEBN
	private String pluginID = "unbbayes.prs.mebn";
	
	// ID of the plugin where we can find new node declarations
	private String pluginNodeExtensionPointID = "MEBNPluginNode";
	...
	public void reloadPlugin() throws IOException {
	    ...
	    PluginDescriptor unbbayesMEBN = this.getUnbbayesPluginContextHolder().getPluginManager().getRegistry().getPluginDescriptor(
	    		this.getPluginID()
	    		);
	    ...
	    ExtensionPoint point = this.getUnbbayesPluginContextHolder().getPluginManager().getRegistry().getExtensionPoint(
	    			unbbayesMEBN.getId(), this.getMainExtensionPointID()
	    		);
