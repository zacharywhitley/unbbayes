...
public class SkeletonPanel extends SchemaPanel {
	 ...
	 * @param layout
	 */
	public static SkeletonPanel newInstance (PRMWindow prmWindow, IPRMController controller, IPRM prm) {
...
//		ret.setLayout(new BorderLayout(5,5));
	...
	protected void initListeners() {
		// we are overwriting this method to allow opening object edition panel instead of class edition panel when an
		// event happens
		...
		this.getPrmClasses().addTreeSelectionListener(new TreeSelectionListener() {
			public void valueChanged(TreeSelectionEvent e) {
				if (e.isAddedPath()) {
					...
					if (prmClass != null) {
						getEditionScrollPane().setViewportView(ObjectPanel.newInstance(SkeletonPanel.this, prmClass));
