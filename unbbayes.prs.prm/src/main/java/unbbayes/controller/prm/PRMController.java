...
public class PRMController implements IPRMController,
	...
	/**
	 * At least one constructor must be visible to subclasses in order to allow
	 * inheritance
	 */
	protected PRMController() {
		...
		// use the same BN inference algorithm between compiler and this
		// controller
		if (this.prmCompiler instanceof IBNInferenceAlgorithmHolder) {
			this.bnInferenceAlgorithm = ((IBNInferenceAlgorithmHolder) this.prmCompiler)
					.getBNInferenceAlgorithm();
			if (this.bnInferenceAlgorithm == null) {
				// if the compiler is not initializing algorithm, I'll do it
				// myself!
				this.bnInferenceAlgorithm = new JunctionTreeAlgorithm();
				((IBNInferenceAlgorithmHolder) this.prmCompiler)
						.setBNInferenceAlgorithm(this.bnInferenceAlgorithm);
			}
		} else {
	 ...
	 * {@link #getPRMModulePluginID()} in order to set up a new instance of
	 * {@link PluginAwareFileExtensionIODelegator} as PRM's default IO
	 * component.
	 * 
	...
	protected BaseIO reloadPluginIO() {
		...
		PluginAwareFileExtensionIODelegator ioDelegator = PluginAwareFileExtensionIODelegator
				.newInstance(false);

		// customize the attributes
		ioDelegator.setCorePluginID(prmModulePluginID);
		ioDelegator.setExtensionPointID(prmIOExtensionPointID);
	 ...
	 * 
	 * @see
	 * unbbayes.controller.prm.IPRMController#setDatabaseController(unbbayes
	 * .controller.prm.IDatabaseController)
	 */
	 ...
	 * 
	 * @see
	 * unbbayes.controller.prm.IPRMController#setPRMCompiler(unbbayes.prs.prm
	 * .compiler.IPRMCompiler)
	 */
	...
	/*
	 * @see
	 * unbbayes.controller.prm.IPRMController#compilePRM(unbbayes.prs.prm.IPRM,
	 * java.util.Collection)
	 */
	public Graph compilePRM(IPRM prm, Collection<IAttributeValue> query) {
		if (prmCompiler != null) {
			return prmCompiler.compile(databaseController,
					prm, query);
		}
		throw new IllegalStateException(
				"The PRM compiler is not initialized. Use "
						+ this.getClass().getName() + ".setPRMCOmpiler("
						+ IPRMCompiler.class.getName() + ") to set it up.");
	}
