...
<%@ page import="br.com.digitaxi.controleacesso.menu.Menu" %>
...
function limpaFormularios() {
	...
	function enviarAssinatura(intUrl) { 
		...
		switch (intUrl) {
			case 0:
				url = "${pageContext.request.contextPath}/Menu/PrepareCreateMenu.do";
				break;
			case 1:
				if (confirm('Deseja apagar os registros selecionados?')) {
					url = "${pageContext.request.contextPath}/Menu/ExcluirMenu.do";
				} else {
					return;
				}
				break;
			case 2:
				url = "${pageContext.request.contextPath}/Menu/PrepareUpdateMenu.do";
				break;
			case 3:
...
<html:form action='Menu/PrepareSelecionarMenu.do' method="POST" >
	...
	<tr>
	  <TD>
	   <TABLE class=fancy width=100% cellpadding=4>
	    <TR>
	     <TD height=25px ><bean:message bundle="Menu" key="controleacesso.menu.nome" />:</TD>
	...
	<tr>
		<td >
			...
			<tr>
				...
				<th bgcolor="ff9900" width='45%'><bean:message bundle="Menu" key="controleacesso.menu.nome" /></th>
				<th bgcolor="ff9900" width='50%'><bean:message bundle="Menu" key="controleacesso.menu.descricao" /></th>
			</tr>
			<%int i=0;%>
			...
			<tr>
				<td align="left">
					<INPUT type="checkbox" name="selecao" value='${item.idMenu}' onclick="habilitaSelecao();"/> 
				</td>
				<td>
					<a style='cursor:pointer;' onclick="document.forms[0].idMenu.value='${item.idMenu}';enviarAssinatura(2);" >${item.nome}</a>
				</td>
				<td >
					<a style='cursor:pointer;' onclick="document.forms[0].idMenu.value='${item.idMenu}';enviarAssinatura(2);" >${item.descricao}</a>
				</td>
			</tr>
			<%} else {%>
			<tr>
