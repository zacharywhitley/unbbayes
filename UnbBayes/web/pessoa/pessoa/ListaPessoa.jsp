...
<%@ page import="br.com.digitaxi.pessoa.pessoa.Pessoa" %>
...
function limpaFormularios() {
	...
	function enviarAssinatura(intUrl) { 
		...
		switch (intUrl) {
			case 0:
				url = "${pageContext.request.contextPath}/Pessoa/PrepareCreatePessoa.do";
				break;
			case 1:
				if (confirm('Deseja apagar os registros selecionados?')) {
					url = "${pageContext.request.contextPath}/Pessoa/ExcluirPessoa.do";
				} else {
					return;
				}
				break;
			case 2:
				url = "${pageContext.request.contextPath}/Pessoa/PrepareUpdatePessoa.do";
				break;
			case 3:
...
<html:form action='Pessoa/PrepareSelecionarPessoa.do' method="POST" >
	...
	<tr>
	  <TD>
	   <TABLE class=fancy width=100% cellpadding=4>
	    <TR>
	     <TD height=25px ><bean:message bundle="Pessoa" key="pessoa.pessoa.nome" />:</TD>
	     <TD height=25px><html:text property="nomePesquisa" styleId="nomePesquisa" styleClass="textbox" size='48' maxlength="45"/></TD>
	    </TR>
	   </TABLE>
	  </TD>
	</tr>
	...
	<tr>
		<td >
			...
			<tr>
				...
				<th bgcolor="ff9900" width='45%'><bean:message bundle="Pessoa" key="pessoa.pessoa.nome" /></th>
				<th bgcolor="ff9900" width='50%'><bean:message bundle="Pessoa" key="pessoa.pessoa.descricao" /></th>
			</tr>
			<%
			int i=0;
			for (Object o : col) {
			...
			<tr>
				<td align="left">
					<INPUT type="checkbox" name="selecao" value='<%=pessoa.getIdPessoa()%>' onclick="habilitaSelecao();"/> 
				</td>
				<td>
					<a style='cursor:pointer;' onclick="document.forms[0].idPessoa.value='<%=pessoa.getIdPessoa()%>';enviarAssinatura(2);" ><%=pessoa.getNome()%></a>
				</td>
				<td >
					<a style='cursor:pointer;' onclick="document.forms[0].idPessoa.value='<%=pessoa.getIdPessoa()%>';enviarAssinatura(2);" ><%=pessoa.getDescricao()%></a>
				</td>
			</tr>
			<%} else {%>
			<tr>
