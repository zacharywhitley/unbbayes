...
<If you do not have JPF in your local mvn repository, then execute the below command>
Go to \mvn_lib\>
mvn install:install-file -DgroupId=guess -DartifactId=jpf -Dversion=1.5 -Dpackaging=jar -Dfile=jpf-1.5.jar

<If you do not have Prognos CPS plugin in your local mvn repository, then execute the below command>
Go to \mvn_lib\prognos>
mvn install:install-file -DpomFile=pom.xml -Dpackaging=jar -Dfile=prognos.unbbayesplugin.cps-1.0.0.jar
