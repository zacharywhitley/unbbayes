...
/**
 * 
 * This is a plugin pane and also this is an imitation of MEBNEditionPane
 * 
 * @author Shou Matsumoto
 *
 */
public class MultiEntityDecisionNodePanelBuilder implements IProbabilityFunctionPanelBuilder {
	 ...
	 * 
	 */
	public MultiEntityDecisionNodePanelBuilder() {
	...
	public JPanel buildProbabilityFunctionEditionPanel() {
		...
		MEDGNodePanel ret = new MEDGNodePanel((MEBNController) mediator, node, getResource().getString("decisionNodeColorLabel"));
		
		// but in decision nodes, we don't need to edit LPDs
		ret.getMEDGNodeToolBar().getBtnEditTable().setEnabled(false);
		ret.getMEDGNodeToolBar().getBtnEditTable().setVisible(false);
		ret.getMEDGNodeToolBar().remove(ret.getMEDGNodeToolBar().getBtnEditTable());
		ret.getMEDGNodeToolBar().setBtnEditTable(null);
