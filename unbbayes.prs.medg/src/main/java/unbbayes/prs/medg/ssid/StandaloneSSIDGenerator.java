 ...
 * to adhere to template method design pattern.
 * @author Shou Matsumoto
 * @deprecated use {@link SSIDGenerator} instead, because it will reflect changes in {@link LaskeySSBNGenerator}
 */
public class StandaloneSSIDGenerator extends LaskeySSBNGenerator implements IMediatorAwareSSBNGenerator {
	...
	private ResourceBundle resourceLog = 
		unbbayes.util.ResourceController.newInstance().getBundle(ResourcesSSBNAlgorithmLog.class.getName(),
				Locale.getDefault(), StandaloneSSIDGenerator.class.getClassLoader()); 
	 ...
	 * @deprecated use {@link SSIDGenerator} instead, because it will reflect changes in {@link LaskeySSBNGenerator}
	 */
	public StandaloneSSIDGenerator(LaskeyAlgorithmParameters parameters) { // NOPMD by Shou Matsumoto on 15/10/16 10:14
		super(parameters);
...
//		this.setBuilderStructure(SSIDBuilderStructure.newInstance());
	 ...
	 * passed as an argument of the constructor {@link #StandaloneSSIDGenerator(LaskeyAlgorithmParameters)}. 
