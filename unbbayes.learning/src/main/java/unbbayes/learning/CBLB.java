...
public class CBLB extends CBLToolkit {
	...
	public CBLB( List<Node> variables, int[][] dataBase, int[] vector,
	...
	private void expand(ArrayList ls) {
		...
		for (int i = 0; i < ls.size(); i++) {
			peace = (double[]) ls.get(i);
			System.out.println("Tentativa = " + (int) peace[1] + ", "
					+ (int) peace[2]);
						...
						if (k != j) {
							...
							if (vars1[1] == vars[1]) {
								for (int w = 0; !flag && w < separators.size(); w++) {
									...
									indexes = (int[]) aux[0];
									sep = (ArrayList) aux[1];
									if (indexes.equals(new int[] { vars[0],
											vars1[0] })) {
										if (notContain(sep, vars[1])) {
											esFinal.add(new int[] { vars[0],
													vars[1] });
											esFinal.add(new int[] { vars1[0],
													vars1[1] });
											remove(new int[] { vars1[0],
													vars1[1] });
