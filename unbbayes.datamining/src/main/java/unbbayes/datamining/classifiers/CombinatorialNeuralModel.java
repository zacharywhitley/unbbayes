 ...
 * @exception Exception if classifier can't be built successfully
  ...
  public void buildClassifier(InstanceSet instanceSet) throws Exception{
	...
	this.classIndex = instanceSet.getClassIndex();      //guarda o indice da classa para serializa��o
  ...
  private void createCombinations(Instance instance, int attributeNum, int classIndex){
	...
	combinationsKeys = makeCombinations(inputKeys);     //cria todas as combina��es dos neuronios de entrada
	combinationsSize = combinationsKeys.length;

	for(int i=0; i<combinationsSize; i++){                    //para cada combina�ao gerada
  ...
  private String[] makeCombinations(String[] inputKeys){
	...
	for(int inputNum=0; inputNum<inputKeysNum; inputNum++){  //para todos os neuronios de entrada
		  ...
		  for (int j = 0; j < combArraySize; j++) {              //para todas as combina��es j� existentes
			...
			tempSize = tempArray.length;                         //pega o tamanho da combina��o

			if (tempSize < maxOrder) {                           //se tamanho da combina��o < ordem m�xima
			  keysArray = new String[tempSize + 1];              //cria nova combina��o
			  keysArray[tempSize] = inputKeys[inputNum];         //adiciona o neuronio de entrada atual
			  System.arraycopy(tempArray, 0, keysArray, 0, tempSize); //copia o resto da combina��o atual
			  combinations.add(keysArray);                       //adiciona nova combina��o no array de combinacoes
			}
		  }
		  keysArray = new String[1];                             //cria nova combina��o de um elemento
		  keysArray[0] = inputKeys[inputNum];                    //coloca o neuronio de entrada atual nesta combina��o
	  combinations.add(keysArray);                           //adiciona nova combina��o no array de combinacoes
	}

	combArraySize = combinations.size();
	combinationsArray = new String[combArraySize];
	for(int i=0; i<combArraySize; i++){                      //gera as chaves das combina��es
  ...
  public Combination[] inference(Instance instance){
	...
	String[] instanceKeys;
	String[] combKeys;
	Combination[] combArray = new Combination[attributeVector[classIndex].numValues()];   //array que conter� os arcos de maior peso de cada neuronio
	int missingAttNum = 0;
	Combination tempCombination;
	...
	combKeys = makeCombinations(instanceKeys);   //gera as chaves de todas as combina��es ativadas por esta instancia
	...
	for(int i=0; i<combKeys.length; i++){
	  if(model.containsKey(combKeys[i])){
		...
		for(int j=0; j<tempOutput.length; j++){
		  if(combArray[j] == null || combArray[j].getOutputNeuron(j) == null){        //testa se o elemento do array de saida � nulo
			combArray[j] = tempCombination;  //se for pega o valor do array tempor�rio
		  } else if(tempOutput[j] != null){  //testa se o elemento do array temporario � nulo
			if(combArray[j].getOutputNeuron(j).getNetWeight() < tempCombination.getOutputNeuron(j).getNetWeight()){  //compara os pesos
			  combArray[j] = tempCombination;  //se o array de saida for menor ent�o pega o temporario.
			}
		  }
		}
	  }
